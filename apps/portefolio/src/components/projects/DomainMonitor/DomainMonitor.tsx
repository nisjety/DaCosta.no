// src/components/projects/DomainMonitor/DomainMonitor.tsx
'use client';
...
const DomainMonitor: React.FC = () => {
  ...
  }, [fetchDomains, handleWebSocketMessage]);
  ...
  const getStatusText = (domain: Domain) => {
    if (!domain.lastStatus) return 'Unknown';
    
    if (domain.lastStatus.isRegistered) {
      return domain.lastStatus.isWebsiteUp ? 'Active' : 'Down';
    } else {
      return 'Available';
    }
  };
                          ...
                          <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex space-x-2">
                              <button
                                ...
                                disabled={loading}
                                className="text-indigo-600 hover:text-indigo-900 transition duration-200 disabled:text-indigo-300"
                              >
                                Check
                              </button>
                              <button
                                ...
                                disabled={loading}
                                className="text-red-600 hover:text-red-900 transition duration-200 disabled:text-red-300"
                              >
                                Delete
                              </button>
                            </div>
